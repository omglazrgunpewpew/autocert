...
function Reset-CircuitBreaker
{
    ...
    if ($OperationName)
    {
        if ($script:CircuitBreakers.ContainsKey($OperationName))
        {
            if ($PSCmdlet.ShouldProcess("Circuit breaker for $OperationName", "Reset"))
            {
    ...
    {
        if ($PSCmdlet.ShouldProcess("All circuit breakers", "Reset"))
        {
            foreach ($name in $script:CircuitBreakers.Keys)
            {
                Reset-CircuitBreaker -OperationName $name -WhatIf:$false -Confirm:$false
            }
        }
    }
}
