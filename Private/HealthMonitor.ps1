...
function Initialize-HealthCheck
{
    ...
    $script:HealthChecks = @{
        ...
        'NetworkConnectivity' = [HealthCheck]::new(
            ...
            {
                $testResults = @()
                $servers = @("acme-v02.api.letsencrypt.org", "dns.google", "one.one.one.one")
                foreach ($server in $servers)
                {
                    try
                    {
                        ...
                        {
                            $testResults += "OK $server"
                        } else
                        {
                            $testResults += "X $server"
                        }
                    } catch
                    {
                        $testResults += "X $server (Error: $($_.Exception.Message))"
                    }
                }
...
function Invoke-HealthCheck
{
    ...
    foreach ($checkName in $checksToRun)
    {
        ...
        if ($Detailed)
        {
            $status = if ($result.Status -eq 'Pass') { 'OK' } else { 'X' }
