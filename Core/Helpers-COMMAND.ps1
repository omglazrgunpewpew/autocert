...
function Set-SecureCredential
{
    ...
    if ($PSCmdlet.ShouldProcess("Credentials for $ProviderName", "Save to $credPath"))
    {
        try
        {
...
function Get-RevokedCertificate
                           {
    [CmdletBinding()]
    param ()
...
function Save-RevokedCertificate
                           {
    [CmdletBinding()]
    param (
        [Parameter(Mandatory = $true)]
