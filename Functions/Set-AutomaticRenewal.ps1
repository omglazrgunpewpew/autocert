        ...
        switch ($choice) {
            ...
            7 {
                ...
                if (Test-RenewalConfig -Config $config) {
                    ...
                    Write-Information -MessageData "✓ Base renewal time: $($baseTime.ToString('HH:mm'))" -InformationAction Continue
                    if ($config.UseRandomization) {
                        $earliestTime = $baseTime.AddMinutes(-$config.RandomizationWindow/2)
                        $latestTime = $baseTime.AddMinutes($config.RandomizationWindow/2)
                        Write-Host -Object "✓ Randomized window: $($earliestTime.ToString('HH:mm')) - $($latestTime.ToString('HH:mm'))" -ForegroundColor Green
                    }
